...
class MarcasSpider(scrapy.Spider):
    name = 'marcas'
    start_urls = ['https://www.olivaoliva.com/es/4-marcas-aceite-de-oliva-virgen-extra-espanolas']

    def parse(self,response):
        marca_urls = response.css('a.subcategory-name::attr(href)').extract()
        for marca_url in marca_urls:
            marca_link = marca_url
            yield scrapy.Request(marca_link, callback=self.get_products)
    ...
    def get_products(self,response):
        ...
        if next_page:
            yield scrapy.Request(next_page,callback=self.get_products)
    ...
    def parse_product(self,response):
        ...
        try:
            ...
            Marca = ''
        try:
            Premios = response.css('div.col-md-6 ::text').extract().strip()
        except:
            Premios = ''
        try:
        ...
        try:
            ...
            meta_description = ''
        try:
            no_reviews = response.css('span[itemprop=reviewCount]::text').get()
        except:
            no_reviews = ''
        try:
            related_products = response.css('section.page-product-box.blockproductscategory li>h5 ::text').extract().replace('€','').strip()
        except:
            related_products = ''
